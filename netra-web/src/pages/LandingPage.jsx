...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen text-slate-800" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* ── TECH STACK STRIP ───────────────────────────────────────────────── */}
      <motion.section className="py-12 relative border-t border-b border-transparent bg-transparent" initial={{ opacity: 0, y: 80 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }} viewport={{ once: true, amount: 0.1 }}>
        ...
        <div className="flex items-center gap-10 px-8 flex-wrap justify-center">
          {["YOLOv8 CV", "React.js", "Leaflet Maps", "PG Portal API", "Recharts", "Tailwind CSS", "FastAPI", "PostGIS", "Redis Queue", "Docker"].map((t) => (
            <span key={t} className="text-xs text-stone-500 font-semibold border border-stone-300 px-3 py-1.5 rounded-full hover:border-[#1e3a8a] hover:text-[#1e3a8a] transition-colors cursor-default">
              {t}
            </span>
          ))}
        </div>
      ...
      {/* ── FOOTER ─────────────────────────────────────────────────────────── */}
      <motion.footer className="py-10 px-8 flex flex-col sm:flex-row items-center justify-between gap-4" style={{ background: '#f5f0eb', borderTop: '1px solid rgba(255,255,255,0.5)' }} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} viewport={{ once: true, amount: 0.2 }}>
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500">
          <a href="mailto:support@codeflex.com" className="flex items-center gap-1.5 text-[#1e3a8a] hover:text-[#1e3a8a] font-semibold transition-colors">
...
function FeatureCard({ icon, tag, title, desc, chips, accent, index }) {
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(255,255,255,0.65)', backdropFilter: 'blur(14px)', borderColor: 'rgba(255,255,255,0.5)' }}
      style={{
...
function PipelineDiagram() {
  ...
  return (
    <div ref={ref} className="mt-16 relative"
      ...
      <div className="rounded-2xl p-8 overflow-x-auto border-transparent bg-transparent">
